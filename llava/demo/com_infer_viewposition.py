...
def infer():
    ...
    print("Creating a subset of samples with 'Nan' view position...")
    df = df[(df['ap_pa'] == 'Nan')].reset_index(drop=True)
    print("Number of samples with 'Nan' view position: {}".format(len(df)))
    ...
    META_PROMPT = "You are an expert radiologist. Your task is to assign a view position for the following X-ray. Give your answer according to the common categories used to define view positions for an X-ray.\n"
    ...
    if(args.num_shards is not None and args.shard is not None):
        filename = "view_position_info_shard_{}.csv".format(args.shard)
    else:
        filename = "view_position_info.csv"
